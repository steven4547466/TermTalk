...
class ClientTUI {
	...
	static run(socket, user) {
		...
		socket.on("disconnect", () => {
			messages.log(`{red-fg}Client > You have been disconnected.{/red-fg}`, this.textPrefix, this.textSuffix)
		})

		socket.on("reconnect", (attempt) => {
			messages.log(`{red-fg}Client > Reconnected after ${attempt} attempt(s).{/red-fg}`, this.textPrefix, this.textSuffix)
		})

		socket.on("reconnect_attempt", (attempt) => {
			messages.log(`{red-fg}Client > Attempting reconnect. #${attempt}{/red-fg}`, this.textPrefix, this.textSuffix)
		})
		...
		socket.on("method_result", (data) => {
			if (!data.success) {
				if (data.method == "messageSend") messages.log(`{red-fg}Client > ${data.message.trim()}{/red-fg}`, this.textPrefix, this.textSuffix)
			}
		})
